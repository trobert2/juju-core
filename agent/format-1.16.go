...
func (formatter *formatter_1_16) winWriteCommands(config *configInternal) ([]string, error) {
	dirName := config.Dir()
	conf := formatter.makeFormat(config)
	data, err := goyaml.Marshal(conf)
	if err != nil {
	...
	commands := winWriteCommandsForFormat(dirName, format_1_16)
	commands = append(commands,
		winWriteFileCommands(formatter.configFile(dirName), string(data), 0600)...)
	return commands, nil
}
