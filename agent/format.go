...
func winWriteFileCommands(filename, contents string, permission int) []string {
	// quotedFilename := utils.ShQuote(filename)
	return []string{
		// fmt.Sprintf("install -m %o /dev/null %s", permission, quotedFilename),
		fmt.Sprintf(`Set-Content "%s" "%s"`, filename, contents),
	}
}

func winWriteCommandsForFormat(dirName, format string) []string {
	commands := []string{"mkdir " + utils.ShQuote(dirName)}
	commands = append(commands, winWriteFileCommands(formatFile(dirName), format, 0644)...)
	return commands
}
